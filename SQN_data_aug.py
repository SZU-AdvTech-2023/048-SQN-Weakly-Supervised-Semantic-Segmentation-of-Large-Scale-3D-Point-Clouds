...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        if(config.name == 'S3DIS'):
            # self.class_weights = DP.get_class_weights('S3DIS')        # 这个函数变了，不是这么用的
        ...
        self.fc1 = pt_utils.Conv2d(2*sum(self.config.d_out), 256, kernel_size=(1,1), bn=True)  # 第一层MLP
        self.fc2 = pt_utils.Conv2d(256, 128, kernel_size=(1,1), bn=True)                      
        self.fc3 = pt_utils.Conv2d(128, 64, kernel_size=(1,1), bn=True)                      
        self.fc4 = pt_utils.Conv2d(64, 64, kernel_size=(1,1), bn=True)
        self.fc5 = pt_utils.Conv2d(64, 32, kernel_size=(1,1), bn=True)
        self.fc6 = pt_utils.Conv2d(32, self.config.num_classes, kernel_size=(1,1), bn=False, activation=None)
    ...
    def forward(self, end_points, device, is_training):     # 先不加数据增强看看效果

        features = end_points['features']  # Batch*channel*npoints
        batch_anno_xyz = end_points['batch_xyz_anno']  # annotated batch xyz    这里的维度是正确的,跟SQN一样
        
        if is_training:
            features = torch.concat([features, self.data_augment(features)], dim=0)  # 在batch维度上进行堆叠
            batch_anno_xyz = torch.concat([batch_anno_xyz, batch_anno_xyz], dim=0)
        else:
            features = features
            batch_anno_xyz = batch_anno_xyz
        ...
        for i in range(self.config.num_layers):
            ...
            # xyz, neigh_idx, sub_idx = end_points['xyz'][i], end_points['neigh_idx'][i], end_points['sub_idx'][i]    
            xyz, neigh_idx, sub_idx = self.input_augment(is_training, end_points['xyz'][i], end_points['neigh_idx'][i], end_points['sub_idx'][i])
            
            
            f_encoder_i = self.dilated_res_blocks[i](features, xyz, neigh_idx)    # 需要用到邻居的索引
            ...
            # ###########################Semantic Query############################
            idx = DP.knn_search(xyz.cpu().numpy(), batch_anno_xyz.cpu().numpy(), 3)                     # 找到附近的3个点的索引（以batch_anno_xyz为中心，在xyz上取3个KNN点）
            idx = torch.tensor(idx, dtype=torch.int64).to(device)
            neighbor_xyz = self.gather_neighbour(xyz, idx)                                              # 用索引找到xyz坐标
            xyz_tile = batch_anno_xyz.unsqueeze(dim=2).repeat(1, 1, idx.shape[-1], 1)
            relative_xyz = xyz_tile - neighbor_xyz                                                      # 计算附近点与查询点的相对坐标
            dist = torch.sum(torch.square(relative_xyz), dim=-1, keepdims=False)                        # 平方取均值，求距离
            weight = torch.ones_like(dist) / 3.0                                                           # xyz的权重，都一样所以是1/3？
            # Trilinear interpolation
            # interpolated_points = three_interpolate(tf.squeeze(f_encoder_i, axis=2), idx, weight)       # 三线性插值
            B = idx.shape[0]
            N = idx.shape[1]
            layer_feature = torch.transpose(f_encoder_i, 1, 2)
            interpolated_points = torch.sum(self.index_points(torch.squeeze(layer_feature, dim=-1), idx) * weight.view(B, N, 3, 1), dim=2)    
            f_interp.append(interpolated_points)
            
           
            
        # Concatenation
        interpolated_points = torch.concat(f_interp, dim=-1)
        interpolated_points = torch.transpose(interpolated_points, 1, 2)        # 将chennel调整回第二个维度
        interpolated_points = torch.unsqueeze(interpolated_points, dim=-1)
        ...
        f_layer_fc1 = self.fc4(interpolated_points)
        f_layer_fc2 = self.fc5(f_layer_fc1)
        f_layer_fc3 = self.fc6(f_layer_fc2)
    ...
    def index_points(points, idx):
        ...
        new_points = points[batch_indices, idx, :]      # 这一步索引有点难理解
        return new_points
    
    @staticmethod
    def input_augment(is_training, xyz, neigh_idx, sub_idx):        
        xyz_c = torch.concat([xyz, xyz], dim=0)
        neigh_idx_c = torch.concat([neigh_idx, neigh_idx], dim=0)
        sub_idx_c = torch.concat([sub_idx, sub_idx], dim=0)
        if is_training:
            element = [xyz_c, neigh_idx_c, sub_idx_c]
        else:
            element = [xyz, neigh_idx, sub_idx]
        
        return element
    
    
    def data_augment(self, data):       # 数据增强
        device = data.device
        data_xyz = data[:, :, 0:3]      # 取出xyz坐标信息
        batch_size = data_xyz.shape[0]
        aug_option = np.random.choice([0, 1, 2])            # 随机选择增强模式

        if aug_option == 0:
            # mirror
            data_xyz = torch.stack([data_xyz[:, :, 0], -data_xyz[:, :, 1], data_xyz[:, :, 2]], dim=2)       # 沿y轴进行镜像翻转?
        elif aug_option == 1:
            # rotation
            ...
            sigma = 0.01
            clip = 0.05
            jittered_point = torch.clamp(sigma * torch.randn(self.config.num_points, 3).to(device), -1 * clip, clip)
            jittered_point = jittered_point.unsqueeze(dim=0).repeat(batch_size, 1, 1)
            data_xyz = data_xyz + jittered_point
                       
        if data.shape[-1] > 3:          # 如果还有颜色信息的话就加上
            data_f = data[:, :, 3:]
            data_aug = torch.concat([data_xyz, data_f], dim=-1)
        else:
            data_aug = data_xyz
                   
        # data_aug_t = torch.transpose(data_aug, 1, 2)
        # data_aug_t = data_aug_t.reshape(-1, data.shape[-1], self.config.num_points)
        # att_activation = self.fc(data_aug)
...
def compute_loss(end_points, cfg, device, is_training):

    logits = end_points['logits']               # 从网络中获取logit和label
    labels = end_points['batch_label_anno']     # 只有某一些点带有标签
